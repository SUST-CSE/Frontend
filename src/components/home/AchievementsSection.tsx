    ...
    <Box sx={{ py: { xs: 8, md: 12 }, bgcolor: '#f8fafc' }}>
      <Container maxWidth="lg">
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', mb: 8 }}>
          <Typography variant="h2" sx={{ fontWeight: 950, color: '#0f172a', fontSize: { xs: '2.5rem', md: '3.5rem' }, letterSpacing: '-0.02em' }}>
        ...
        <Grid container spacing={4}>
          {achievements.slice(0, 3).map((achievement: IAchievement) => (
            <Grid key={achievement._id.toString()} size={{ xs: 12, md: 4 }}>
              <Paper
                elevation={0}
                sx={{
                  height: '100%',
              ...
              >
                {/* Photo Section */}
                <Box sx={{ position: 'relative', height: 260, bgcolor: '#f1f5f9' }}>
                  <Image 
