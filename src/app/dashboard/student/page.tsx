...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    if (user) {
      // Get the current token from cookies to prevent it from being lost
                ...
                <Grid size={{ xs: 12, md: 4 }}>
                  <Paper elevation={0} sx={{ p: 3, borderRadius: 4, textAlign: 'center', border: '1px solid #e2e8f0', bgcolor: '#f8fafc' }}>
                    ...
                    <Stack spacing={2} sx={{ textAlign: 'left' }}>
                      ...
                      <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                        <Box sx={{ p: 1, borderRadius: 1.5, bgcolor: '#fff', color: '#64748b', border: '1px solid #e2e8f0' }}><LucideGraduationCap size={16} /></Box>
                        <Typography variant="body2" fontWeight={600}>ID: {user.studentId}</Typography>
                      </Box>
                    </Stack>
                  </Paper>
                </Grid>

                {/* Academic Details & Quick Actions */}
                <Grid size={{ xs: 12, md: 8 }}>
                  <Stack spacing={4}>
